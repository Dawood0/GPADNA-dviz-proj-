...
class BeeswarmData:
    ...
    feature_configs_by_key: dict[str, FeatureConfig]
...
def prepare_beeswarm_data(
    ...
    """Prepare reusable beeswarm metadata once when the app starts."""
    valid_features = [feature for feature in features if feature in df.columns]
    feature_configs = _build_feature_configs(df, valid_features)
    ...
    return BeeswarmData(
        ...
        feature_configs_by_key={config.key: config for config in feature_configs},
    )


def create_visual(data: BeeswarmData | pd.DataFrame, **kwargs) -> tuple[go.Figure, str, str]:
    """Create the beeswarm figure.

    Prefer passing BeeswarmData from prepare_beeswarm_data() so feature
    detection/configuration is not repeated on every Dash callback. Passing a
    raw DataFrame still works as a fallback.
    """
    if isinstance(data, BeeswarmData):
        df = data.df
        default_features = data.features
        target_col = kwargs.get("target_col", data.target_col)
        student_id_col = kwargs.get("student_id_col", data.student_id_col)
        feature_configs_by_key = data.feature_configs_by_key
    else:
        df = data
        detected = detect_columns(df)
        default_features = detected["features"]
        target_col = kwargs.get("target_col", detected["target_col"])
        student_id_col = kwargs.get("student_id_col", detected["student_id_col"])
        feature_configs_by_key = {
            config.key: config
            for config in _build_feature_configs(df, default_features)
        }
    ...
    feature_configs = [
        feature_configs_by_key[feature]
